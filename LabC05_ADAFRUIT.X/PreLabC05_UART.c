 /* Archivo: PostLabC05.c
 *Dispositivo: PIC16F887
 * Autor: Kevin Alarcón
 * Compilador: XC8(v2.40), MPLABX V6.05
 * 
 * 
 * Programa: Mandar y recibir datos desde adafruit
 * Hardware: Botones en puerto B y Leds en los puertos A y D
 * 
...
void POT(uint8_t contador, char puerto);
void TXT(void);
/******************************INTERRUPCIONES*******************************/
...
void __interrupt() isr(void)
{
    ...
    if (INTCONbits.RBIF) //Revisamos si la bandera de interrupción del Puerto B se enciende
    {
        ...
        //RB2 -> Presentar el valor ingresado en la terminal en el puerto C
        else if (PORTBbits.RB2 == 0){
            POT(contador, 'A');
        }
        //RB2 -> Presentar el valor del contador en la terminal
...
{/*IO*/
    ...
    //Puertos que será entradas
    TRISB = 0x0f;
    
    //Puertos que serán salidas
    TRISA = 0;    
    TRISD = 0;
    TRISE = 0;
...
{       //ADON = 1; //se enciende el ADC
        ...
        sprintf(CVPOT, "%d\r", contador); //Pasa a el valor del potenciometro a caracter
        sprintf(oracion, "El valor del puerto %c es:", puerto);
        //cadena(oracion); //Presentamos el valor del potenciometro
